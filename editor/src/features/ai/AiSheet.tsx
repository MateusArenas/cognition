...
function sugestoesPara(sel: Selection | null): string[] {
  if (!sel) return ['Deixe as cores mais consistentes', 'Simplifique os rótulos'];
  if (sel.kind === 'edge') return ['Inverta o sentido'];
  if (sel.kind === 'table') return ['Adicione campos de auditoria (criado_em, atualizado_em)'];
  if (sel.kind === 'col') return ['Sugira um tipo melhor'];
  if (sel.kind === 'node') return ['Deixe o texto mais curto'];
  return [];
}
...
export const AiSheet = forwardRef<BottomSheetModal, Props>(function AiSheet({ doc, sel, onValidate, onApply }, ref) {
  ...
  return (
    <Sheet ref={ref} title="Pedir para a IA">
      <View style={{ gap: space.lg }}>
        {sel ? (
          <Segmented
            options={[
              { value: 'doc', label: 'Diagrama inteiro' },
              { value: 'elemento', label: 'Elemento selecionado' },
            ]}
            value={escopo}
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          {sugestoesPara(escopo === 'elemento' ? sel : null).map((s) => (
            <Chip key={s} label={s} onPress={() => gerar(s)} />
          ))}
        </View>

        <Field value={pedido} onChangeText={setPedido} placeholder="O que você quer mudar?" multiline />

        {loading ? <ActivityIndicator color={colors.blue} /> : null}
        {erro ? <Text style={{ color: colors.red, fontSize: 13 }}>{erro}</Text> : null}

        <Chip label={loading ? 'Gerando…' : 'Gerar'} onPress={() => gerar()} />
      </View>
    </Sheet>
  );
});
